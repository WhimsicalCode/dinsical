...
async function loadAndBuild(): Promise<void> {
  ...
  try {
    ...
    S.pair = pair
    const d = pair.dinsy
    const c = pair.dinsical
    $status.textContent =
      `Dinsy — UPM ${d.unitsPerEm}  asc ${d.ascender}  desc ${d.descender}` +
      `   ·   ` +
      `DINsical — UPM ${c.unitsPerEm}  asc ${c.ascender}  desc ${c.descender}`
    buildGrid(pair)
  } catch (err) {
...
function openModal(char: string): void {
  ...
  const i2 = getGlyphInfo(char, pair.dinsical)
  ...
  $mMetrics.innerHTML = `
    ...
    <div class="mcard" style="border-top:3px solid #2563eb">
      <div class="mcard-title" style="color:#2563eb">DINsical</div>
