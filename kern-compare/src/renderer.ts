...
export const DINSICAL_COLOR = '#2563eb'
...
function cellHeightFromPair(pair: FontPair, fontSize: number): { cellH: number; baseline: number } {
  ...
  const s2 = scale(pair.dinsical, fontSize)
...
export function renderStringOnCanvas(
  ...
  const { cellH, baseline } = cellHeightFromPair({ dinsy: font, dinsical: font }, opts.fontSize)
...
export function renderPairOverlay(
  ...
  const adv2 = pairAdvancePx(pair.dinsical, leftCP, rightCP, opts.fontSize)
  ...
  const text = String.fromCodePoint(leftCP) + String.fromCodePoint(rightCP)
  // DINsical underneath, Dinsy on top
  strokeText(ctx, pair.dinsical, text, (w - adv2) / 2, baseline, opts.fontSize, DINSICAL_COLOR, opts.lineWidth)
