...
function buildGrid(): void {
  ...
  const filtered = filterPairs(allPairs, S.dinsyMap, S.dinsicalMap, S.filter, S.range)
...
function fakeCellDims(pair: FontPair, fontSize: number): { cellH: number; baseline: number } {
  ...
  const s2 = fontSize / pair.dinsical.unitsPerEm
  const asc  = Math.max(pair.dinsy.ascender   * s1, pair.dinsical.ascender   * s2)
  const desc = Math.max(-pair.dinsy.descender * s1, -pair.dinsical.descender * s2)
...
function makePairCell(
  ...
  const cv = lookupKern(S.dinsicalMap, leftCP, rightCP)

  if (S.viewMode === 'overlay') {
    const canvas = document.createElement('canvas')
    // Reserve space so layout doesn't jump when rendered
    const adv1 = pairAdvancePx(pair.dinsy,    leftCP, rightCP, S.fontSize)
    const adv2 = pairAdvancePx(pair.dinsical, leftCP, rightCP, S.fontSize)
    ...
    scheduleRender(c2, () => renderPairOnCanvas(c2, leftCP, rightCP, pair.dinsical, DINSICAL_COLOR, opts()))
    wrap.appendChild(c1)
    wrap.appendChild(c2)
  ...
  vc.className = `kv dinsical${cv === 0 ? ' zero' : ''}`
...
function openModal(leftCP: number, rightCP: number): void {
  ...
  const g1c = pair.dinsical.charToGlyph(l)
  const g2c = pair.dinsical.charToGlyph(r)
  ...
  $mMetrics.innerHTML = `
    ...
    <div class="mcard mcard-delta">
      ...
      <div class="mrow"><span>status</span><code class="diff">${dv === 0 ? 'missing in Dinsy' : cv === 0 ? 'missing in DINsical' : 'values differ'}</code></div>
    </div>` : ''}
  `
